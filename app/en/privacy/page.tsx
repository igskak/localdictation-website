...
export default function PrivacyPage() {
  ...
  const tools = analytics && ads
    ...
    : analytics
      ...
      : <>the <strong>Google Ads conversion tag</strong></>;
  return <LegalShell
    locale="en"
    page="privacy"
    eyebrow="Privacy"
  ...
  >
    ...
    {product && <><p><strong>What the service reports to PostHog</strong>: four facts about the business, each when it happens — a trial was issued, a licence was bought, a licence renewed, a licence was refunded. They go to the same PostHog project in PostHog&apos;s <em>EU Cloud</em> that measures this site (section 8), so trials and sales can be counted next to visits and downloads. Each event carries a random identifier the service creates for the licence, the kind of licence, the payment provider&apos;s name, whether a purchase was an upgrade, and — for payments — the amount and currency. <strong>It carries no e-mail address, no device identifier and no IP address</strong>, and it creates no person profile in PostHog, so an event says that a licence was sold, not who bought it. The legal basis is Art. 6(1)(f) GDPR; our legitimate interest is knowing how many trials and sales there are over time. The events are deleted on the same terms as this site&apos;s PostHog events.</p></>}
