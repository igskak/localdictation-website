...
export default function DatenschutzPage() {
  ...
  >
    ...
    {product && <><p><strong>Was der Dienst an PostHog meldet</strong>: vier Tatsachen über das Geschäft, jeweils in dem Moment, in dem sie eintreten — ein Test wurde ausgestellt, eine Lizenz wurde gekauft, eine Lizenz wurde verlängert, eine Lizenz wurde erstattet. Sie gehen an dasselbe PostHog-Projekt in der <em>EU Cloud</em> von PostHog, das diese Website misst (Abschnitt 8), damit Tests und Verkäufe neben Besuchen und Downloads gezählt werden können. Jedes Ereignis enthält eine zufällige Kennung, die der Dienst für die Lizenz erzeugt, die Art der Lizenz, den Namen des Zahlungsdienstleisters, ob ein Kauf ein Upgrade war, und — bei Zahlungen — Betrag und Währung. <strong>Es enthält keine E-Mail-Adresse, keine Gerätekennung und keine IP-Adresse</strong> und legt in PostHog kein Personenprofil an; ein Ereignis sagt also, dass eine Lizenz verkauft wurde, nicht wer sie gekauft hat. Rechtsgrundlage ist Art. 6 Abs. 1 lit. f DSGVO; unser berechtigtes Interesse ist zu wissen, wie viele Tests und Verkäufe es im Zeitverlauf gibt. Die Ereignisse werden unter denselben Bedingungen gelöscht wie die PostHog-Ereignisse dieser Website.</p></>}
    ...
    <p><strong>Ein eingebundenes Bild von einem fremden Server</strong>: Im Vertrauensband der Startseite steht das Abzeichen von <strong>Product Hunt</strong>. Es liegt nicht bei uns, sondern wird von <code>api.producthunt.com</code> geladen, damit die Stimmenzahl darauf die aktuelle ist. Dein Browser ruft es beim Laden der Seite ab; Product Hunt erfährt dabei die Verbindungsdaten dieses Abrufs — IP-Adresse, Zeitpunkt, Browser und die Seite, von der aus abgerufen wird — und setzt dabei ein Cookie von Cloudflare (<code>__cf_bm</code>, rund eine halbe Stunde) zur Abwehr automatisierter Abrufe. Anbieter ist die Product Hunt, Inc. in den Vereinigten Staaten; es gilt deren Datenschutzerklärung, und ein Zugriff dortiger Behörden lässt sich nicht ausschließen. Rechtsgrundlage ist Art. 6 Abs. 1 lit. f DSGVO; unser berechtigtes Interesse ist, die Bewertung des Produkts dort zu zeigen, wo sie abgegeben wurde. Du kannst dem nach Art. 21 DSGVO widersprechen. Das Abzeichen misst nichts, erkennt dich nicht wieder und erfährt nichts über deinen Besuch außer diesem einen Abruf.</p>
    <p>Sonst bindet die Seite keine Schriften, Karten oder Videos von fremden Servern ein.</p>
